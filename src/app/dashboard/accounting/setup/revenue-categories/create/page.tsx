  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error || 'Failed to create revenue category');
      }
      router.push('/dashboard/accounting/setup/revenue-categories');
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Failed to create revenue category');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="p-6 bg-gray-900 min-h-screen">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-white">Create Revenue Categories</h1>
          <p className="text-sm text-gray-400 mt-1">Add a new revenue category</p>
        </div>
      </div>
