...
export default function SystemSetupPage() {
  ...
  const [selectedItem, setSelectedItem] = useState<AccountType | null>(null);
  ...
  const fetchAccountTypes = async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/accounting/setup?type=accountType');
  ...
  useEffect(() => { (async () => { await fetchAccountTypes(); })(); }, []);

  const handleView = (item: AccountType) => {
  ...
  const handleEdit = (item: AccountType) => {
    ...
    setEditFormData({
      ...
      normalBalance: item.attributes?.normalBalance || 'Debit',
  ...
  const handleDelete = (item: AccountType) => {
    setSelectedItem(item);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setAccountTypes(prev => prev.filter(c => c.id !== selectedItem.id));
    } catch {
      // ignore
    } finally {
      setDeleting(false);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const res = await fetch(`/api/accounting/setup/${editFormData.id}`, {
        ...
        body: JSON.stringify({
          ...
          attributes: {
            ...
            normalBalance: editFormData.normalBalance,
      ...
      if (res.ok) {
        ...
        setAccountTypes(prev => prev.map(c => (c.id === updated.id ? updated : c)));
      }
    } catch {
      // ignore
    } finally {
  ...
  const filteredItems = accountTypes.filter(item => {
    const q = searchTerm.toLowerCase();
    const attrs = item.attributes || {};
    return (
      item.name.toLowerCase().includes(q) ||
      (attrs.code?.toLowerCase() || '').includes(q) ||
      (attrs.category?.toLowerCase() || '').includes(q)
    );
  });
  ...
  const getCategoryColor = (category: string) => {
    const colors: Record<string, string> = {
      'Assets': 'bg-blue-900/50 text-blue-400 border border-blue-800',
      'Liabilities': 'bg-yellow-900/50 text-yellow-400 border border-yellow-800',
      ...
      'Expenses': 'bg-red-900/50 text-red-400 border border-red-800',
    };
    return colors[category] || 'bg-gray-700 text-gray-400 border border-gray-600';
  };
  ...
  return (
    <div className="p-6 bg-gray-900 min-h-screen">
      <div className="flex items-center gap-2 text-sm text-gray-400 mb-4">
        ...
        <span className="text-white">Account Types</span>
      </div>

      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-white">System Setup</h1>
          <p className="text-sm text-gray-400 mt-1">Manage account types and categories</p>
        </div>
        <Link href="/dashboard/accounting/setup/account-types/create">
      ...
      <div className="flex gap-1 bg-gray-800 rounded-lg border border-gray-700 p-1 mb-6">
        <button className="px-4 py-2 bg-blue-600 text-white rounded-md text-sm font-medium">Account Types</button>
        <Link href="/dashboard/accounting/setup/revenue-categories">
          <button className="px-4 py-2 text-gray-400 hover:text-white hover:bg-gray-700 rounded-md text-sm font-medium transition">Revenue Categories</button>
        </Link>
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex-1 min-w-[200px]">
            <div className="relative">
              ...
              <input type="text" placeholder="Search account types..."
          ...
          <table className="w-full">
            <thead className="bg-gray-900">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider cursor-pointer hover:text-white" onClick={() => handleSort('name')}>
                  <div className="flex items-center gap-1"><Tag className="w-3 h-3" /> Name <SortIcon field="name" sortField={sortField} sortDirection={sortDirection} /></div>
                </th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider cursor-pointer hover:text-white" onClick={() => handleSort('code')}>
                  <div className="flex items-center gap-1"><Hash className="w-3 h-3" /> Code <SortIcon field="code" sortField={sortField} sortDirection={sortDirection} /></div>
                </th>
                <th className="px-4 py-3 text-center text-xs font-medium text-gray-400 uppercase tracking-wider cursor-pointer hover:text-white" onClick={() => handleSort('normalBalance')}>
                  Normal Balance <SortIcon field="normalBalance" sortField={sortField} sortDirection={sortDirection} />
                </th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider cursor-pointer hover:text-white" onClick={() => handleSort('category')}>
                  <div className="flex items-center gap-1"><Layers className="w-3 h-3" /> Category Name <SortIcon field="category" sortField={sortField} sortDirection={sortDirection} /></div>
                </th>
                <th className="px-4 py-3 text-center text-xs font-medium text-gray-400 uppercase tracking-wider cursor-pointer hover:text-white" onClick={() => handleSort('isActive')}>
                  Is Active <SortIcon field="isActive" sortField={sortField} sortDirection={sortDirection} />
                </th>
            ...
            <tbody className="divide-y divide-gray-700">
              {loading ? (
                ...
                <tr><td colSpan={6} className="px-4 py-12 text-center text-gray-500">No account types found</td></tr>
              ) : (
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-gray-700/50 transition">
                      ...
                      <td className="px-4 py-4 whitespace-nowrap text-center">
                        <span className={`px-2 py-1 text-xs rounded-full ${getBalanceColor(attrs.normalBalance || 'Debit')}`}>
                          {attrs.normalBalance || 'Debit'}
                        </span>
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 text-xs rounded-full ${getCategoryColor(attrs.category || '')}`}>
                          {attrs.category || '—'}
                        </span>
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap text-center">
                        {item.isActive ? (
                          <span className="flex items-center justify-center gap-1 text-green-400"><CheckCircle className="w-4 h-4" /><span className="text-xs">Active</span></span>
                        ) : (
                          <span className="flex items-center justify-center gap-1 text-red-400"><XCircle className="w-4 h-4" /><span className="text-xs">Inactive</span></span>
                        )}
                      </td>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-lg border border-gray-700 max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-white">Account Type Details</h2>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-xs text-gray-500">Normal Balance</label>
                  <span className={`px-2 py-1 text-xs rounded-full ${getBalanceColor(selectedItem.attributes?.normalBalance || 'Debit')}`}>
                    {selectedItem.attributes?.normalBalance || 'Debit'}
                  </span>
                </div>
              </div>
              <div>
                <label className="text-xs text-gray-500">Category</label>
                <span className={`px-2 py-1 text-xs rounded-full ${getCategoryColor(selectedItem.attributes?.category || '')}`}>
      ...
      {showEditModal && editFormData && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-lg border border-gray-700 max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-white">Edit Account Type</h2>
                ...
                <div className="flex gap-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="normalBalance" value="Debit"
                      checked={editFormData.normalBalance === 'Debit'} onChange={handleEditChange}
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="normalBalance" value="Credit"
                      checked={editFormData.normalBalance === 'Credit'} onChange={handleEditChange}
                      className="w-4 h-4 bg-gray-900 border-gray-700 text-blue-600 focus:ring-blue-500" />
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-lg border border-gray-700 max-w-md w-full p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-xl font-semibold text-white">Delete Account Type</h2>
            </div>
            <p className="text-gray-400 mb-2">Are you sure you want to delete this account type?</p>
            <div className="bg-gray-900 rounded-lg p-3 mb-4">
              ...
              <p className="text-sm text-gray-400">Category: {selectedItem.attributes?.category || '—'}</p>
            </div>
