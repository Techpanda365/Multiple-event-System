...
import {
  ...
  Eye, Edit, Save, X, BookOpen, User, Building2, Receipt
} from 'lucide-react';
import Link from 'next/link';

interface DebitNoteRaw {
  id: string;
...
interface PurchaseReturn { id: string; returnNumber: string; }
...
export default function DebitNotePage() {
  const [notes, setNotes] = useState<DebitNoteRaw[]>([]);
  ...
  const [selectedNote, setSelectedNote] = useState<DebitNoteRaw | null>(null);
  const [editFormData, setEditFormData] = useState<EditFormData | null>(null);

  const vendorMap = new Map(vendors.map(v => [v.id, v.name]));
  const prMap = new Map(purchaseReturns.map(p => [p.id, p.returnNumber]));

  const fetchAll = async () => {
    try {
      setLoading(true);
      const [notesRes, vendorsRes, prRes] = await Promise.all([
        fetch('/api/accounting/debit-note'),
        fetch('/api/purchase/vendors'),
        fetch('/api/accounting/purchase-returns'),
      ]);
      if (notesRes.ok) setNotes(await notesRes.json());
      if (vendorsRes.ok) setVendors(await vendorsRes.json());
      if (prRes.ok) setPurchaseReturns(await prRes.json());
    } catch (err) {
      console.error('Failed to fetch debit note data:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleView = (r: DebitNoteRaw) => { setSelectedNote(r); setShowViewModal(true); };
  const handleEdit = (r: DebitNoteRaw) => {
    setSelectedNote(r);
    setEditFormData({ id: r.id, date: r.date.split('T')[0], totalAmount: r.totalAmount, balance: r.balance, notes: r.notes || '' });
    setShowEditModal(true);
  };
  const handleDelete = (r: DebitNoteRaw) => { setSelectedNote(r); setShowDeleteModal(true); };

  const confirmDelete = async () => {
    if (!selectedNote) return;
    setDeletingId(selectedNote.id);
    try {
      await fetch(`/api/accounting/debit-note/${selectedNote.id}`, { method: 'DELETE' });
  ...
  const handleStatusUpdate = async (id: string, status: string) => {
    setUpdatingId(id);
    try {
      const res = await fetch(`/api/accounting/debit-note/${id}`, {
        method: 'PATCH',
  ...
  const handleSaveEdit = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/accounting/debit-note/${editFormData.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const enhanced = notes.map(n => ({
    ...n,
    vendorName: n.vendorId ? vendorMap.get(n.vendorId) || '—' : '—',
    returnNumber: n.purchaseReturnId ? prMap.get(n.purchaseReturnId) || '—' : '—',
    approvedByName: n.approvedById ? usersMap[n.approvedById] || '—' : '—',
  }));

  const filtered = enhanced.filter(r => {
    const q = searchTerm.toLowerCase();
    return (r.debitNoteNumber?.toLowerCase() || '').includes(q) || r.vendorName.toLowerCase().includes(q) || r.returnNumber.toLowerCase().includes(q);
  });
  ...
  const sorted = [...currentItems].sort((a, b) => {
    ...
    switch (sortField) {
      case 'debitNoteNumber': aVal = a.debitNoteNumber || ''; bVal = b.debitNoteNumber || ''; break;
  ...
  return (
    <div className="p-6 bg-gray-900 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <Link href="/dashboard/accounting/debit-note/create">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex-1 min-w-[200px]">
            <div className="relative">
              ...
              <input type="text" placeholder="Search debit notes..." className="w-full pl-10 pr-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
          ...
          <table className="w-full">
            <thead className="bg-gray-900">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider cursor-pointer hover:text-white" onClick={() => handleSort('debitNoteNumber')}>
                  <div className="flex items-center gap-1"><Hash className="w-3 h-3" /> Debit Note # <SortIcon field="debitNoteNumber" sortField={sortField} sortDirection={sortDirection} /></div>
                </th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">
                  <div className="flex items-center gap-1"><Receipt className="w-3 h-3" /> Purchase Return</div>
                </th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider cursor-pointer hover:text-white" onClick={() => handleSort('vendor')}>
                  <div className="flex items-center gap-1"><Building2 className="w-3 h-3" /> Vendor <SortIcon field="vendor" sortField={sortField} sortDirection={sortDirection} /></div>
                </th>
                ...
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-400 uppercase tracking-wider">
                  <div className="flex items-center justify-end gap-1"><BookOpen className="w-3 h-3" /> Balance</div>
                </th>
            ...
            <tbody className="divide-y divide-gray-700">
              {loading ? (
                ...
                <tr><td colSpan={9} className="px-4 py-12 text-center text-gray-500">No debit notes found</td></tr>
              ) : (
                sorted.map((r) => (
                  <tr key={r.id} className="hover:bg-gray-700/50 transition">
                    <td className="px-4 py-4 whitespace-nowrap"><span className="text-sm font-mono text-blue-400">{r.debitNoteNumber || '—'}</span></td>
                    <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-400">{r.returnNumber}</td>
                    <td className="px-4 py-4 whitespace-nowrap text-sm text-white">{r.vendorName}</td>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-white font-mono text-lg">{selectedNote.debitNoteNumber || '—'}</p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-white">{selectedNote.purchaseReturnId ? prMap.get(selectedNote.purchaseReturnId) || '—' : '—'}</p>
                </div>
                <div>
                  <label className="text-xs text-gray-500">Vendor</label>
                  <p className="text-white">{selectedNote.vendorId ? vendorMap.get(selectedNote.vendorId) || '—' : '—'}</p>
                </div>
              </div>
              <div>
      ...
      {showEditModal && editFormData && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-lg border border-gray-700 max-w-lg w-full p-6">
            ...
            <div className="space-y-4">
              <p className="text-sm text-gray-400 font-mono">{selectedNote?.debitNoteNumber || ''}</p>
              <div>
      ...
      {showDeleteModal && selectedNote && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-lg border border-gray-700 max-w-md w-full p-6">
            <h2 className="text-xl font-semibold text-white mb-4">Delete Debit Note</h2>
            <p className="text-gray-400 mb-2">Are you sure?</p>
            <div className="bg-gray-900 rounded-lg p-3 mb-4">
              <p className="text-sm text-white font-mono">{selectedNote.debitNoteNumber || 'Debit Note'}</p>
              <p className="text-sm text-gray-400">{selectedNote.vendorId ? vendorMap.get(selectedNote.vendorId) || '—' : '—'} — {fmt(selectedNote.totalAmount)}</p>
            </div>
