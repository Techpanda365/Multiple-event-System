...
export default function CreateRevenuePage() {
    ...
    const fetchData = async () => {
      try {
        const [bankRes, coaRes, catRes] = await Promise.all([
          ...
          fetch('/api/accounting/setup?type=revenueCategory'),
        ]);
        if (bankRes.ok) setBankAccounts(await bankRes.json());
        if (coaRes.ok) setChartOfAccounts(await coaRes.json());
        if (catRes.ok) {
          const cats = await catRes.json();
          setRevenueCategories(cats.map((c: { id: string; name: string }) => ({ id: c.id, name: c.name })));
        }
      } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.date) { setError('Revenue date is required'); return; }
    ...
    try {
      ...
      router.push('/dashboard/accounting/revenue');
    } catch (err: unknown) {
  ...
  return (
    <div className="p-6 bg-gray-900 min-h-screen">
      <div className="flex items-center gap-4 mb-6">
        <Link href="/dashboard/accounting/revenue">
          <button className="p-2 bg-gray-800 rounded-lg hover:bg-gray-700 transition">
            <ChevronLeft className="w-5 h-5 text-gray-400" />
          </button>
        </Link>
        <div>
          <h1 className="text-2xl font-semibold text-white">Create Revenue</h1>
          <p className="text-sm text-gray-400 mt-1">Record incoming revenue</p>
        </div>
      </div>
