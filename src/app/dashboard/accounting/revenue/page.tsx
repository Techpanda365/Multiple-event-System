...
export default function RevenuePage() {
  ...
  const fetchRevenues = async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/accounting/revenue');
      if (res.ok) {
  ...
  const confirmDelete = async () => {
    ...
    setDeletingId(selectedRevenue.id);
    try {
      await fetch(`/api/accounting/revenue/${selectedRevenue.id}`, { method: 'DELETE' });
  ...
  const handleStatusUpdate = async (id: string, status: string) => {
    setUpdatingId(id);
    try {
      const res = await fetch(`/api/accounting/revenue/${id}`, {
        method: 'PATCH',
  ...
  const handleSaveEdit = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/accounting/revenue/${editFormData.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const sorted = [...currentItems].sort((a, b) => {
    ...
    switch (sortField) {
      case 'revenueNumber': aVal = a.revenueNumber; bVal = b.revenueNumber; break;
  ...
  return (
    <div className="p-6 bg-gray-900 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-white">Manage Revenues</h1>
          <p className="text-sm text-gray-400 mt-1">Track all incoming revenue</p>
        </div>
        <Link href="/dashboard/accounting/revenue/create">
          <button className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-blue-700 transition">
            <Plus className="w-4 h-4" /> Create Revenue
          </button>
        </Link>
      </div>
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex-1 min-w-[200px]">
            <div className="relative">
              ...
              <input type="text" placeholder="Search revenues..." className="w-full pl-10 pr-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-900">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider cursor-pointer hover:text-white" onClick={() => handleSort('revenueNumber')}>
                  <div className="flex items-center gap-1"><Hash className="w-3 h-3" /> Revenue # <SortIcon field="revenueNumber" sortField={sortField} sortDirection={sortDirection} /></div>
                </th>
            ...
            <tbody className="divide-y divide-gray-700">
              {loading ? (
                ...
                <tr><td colSpan={9} className="px-4 py-12 text-center text-gray-500">No revenues found</td></tr>
              ) : (
                sorted.map((r) => (
                  <tr key={r.id} className="hover:bg-gray-700/50 transition">
                    <td className="px-4 py-4 whitespace-nowrap"><span className="text-sm font-mono text-blue-400">{r.revenueNumber}</span></td>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-lg border border-gray-700 max-w-lg w-full p-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => { setShowViewModal(false); setSelectedRevenue(null); }} className="p-1 hover:bg-gray-700 rounded-lg transition"><X className="w-5 h-5 text-gray-400" /></button>
            </div>
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <div>
                  <label className="text-xs text-gray-500">Revenue Number</label>
                  <p className="text-white font-mono text-lg">{selectedRevenue.revenueNumber}</p>
                </div>
                <span className={`inline-flex items-center gap-1 px-2 py-1 text-xs rounded-full ${badge(selectedRevenue.status)}`}>
                  {statusIcon(selectedRevenue.status)}{selectedRevenue.status}
                </span>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs text-gray-500">Date</label>
                  <p className="text-white">{new Date(selectedRevenue.date).toLocaleDateString()}</p>
                </div>
                <div>
                  <label className="text-xs text-gray-500">Category</label>
                  <p className="text-white">{selectedRevenue.category}</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs text-gray-500">Bank Account</label>
                  <p className="text-white">{selectedRevenue.bankAccount?.name || '—'}</p>
                </div>
                <div>
                  <label className="text-xs text-gray-500">Chart of Account</label>
                  <p className="text-white">{selectedRevenue.chartOfAccount ? `${selectedRevenue.chartOfAccount.code} - ${selectedRevenue.chartOfAccount.name}` : '—'}</p>
                </div>
              </div>
              <div>
                <label className="text-xs text-gray-500">Amount</label>
                <p className="text-white text-xl font-semibold">{fmt(selectedRevenue.amount)}</p>
              </div>
              {selectedRevenue.referenceNumber && (
                <div>
                  <label className="text-xs text-gray-500">Reference Number</label>
                  <p className="text-white">{selectedRevenue.referenceNumber}</p>
                </div>
              )}
              {selectedRevenue.description && (
                <div>
                  <label className="text-xs text-gray-500">Description</label>
                  <p className="text-gray-300">{selectedRevenue.description}</p>
                </div>
              )}
            </div>
            <div className="mt-6 flex justify-end">
              <button onClick={() => { setShowViewModal(false); setSelectedRevenue(null); }} className="px-4 py-2 bg-gray-700 text-white rounded-lg hover:bg-gray-600 transition">Close</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showEditModal && editFormData && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-lg border border-gray-700 max-w-lg w-full p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-white">Edit Revenue</h2>
              <button onClick={() => { setShowEditModal(false); setSelectedRevenue(null); setEditFormData(null); }} className="p-1 hover:bg-gray-700 rounded-lg transition"><X className="w-5 h-5 text-gray-400" /></button>
            </div>
            <div className="space-y-4">
              <p className="text-sm text-gray-400 font-mono">{selectedRevenue?.revenueNumber}</p>
              <div className="grid grid-cols-2 gap-4">
                <div>
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button onClick={() => { setShowEditModal(false); setSelectedRevenue(null); setEditFormData(null); }}
      ...
      {showDeleteModal && selectedRevenue && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-lg border border-gray-700 max-w-md w-full p-6">
            <h2 className="text-xl font-semibold text-white mb-4">Delete Revenue</h2>
            <p className="text-gray-400 mb-2">Are you sure?</p>
            <div className="bg-gray-900 rounded-lg p-3 mb-4">
              <p className="text-sm text-white font-mono">{selectedRevenue.revenueNumber}</p>
              <p className="text-sm text-gray-400">{selectedRevenue.category} — {fmt(selectedRevenue.amount)}</p>
            </div>
            <div className="flex justify-end gap-3">
              <button onClick={() => { setShowDeleteModal(false); setSelectedRevenue(null); }}
                className="px-4 py-2 border border-gray-700 rounded-lg text-gray-400 hover:bg-gray-700 transition">Cancel</button>
              <button onClick={confirmDelete} disabled={deletingId === selectedRevenue.id}
                className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition flex items-center gap-2 disabled:opacity-50">
                {deletingId === selectedRevenue.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />} Delete
              </button>
            </div>
          </div>
        </div>
