...
export default function VendorPaymentsPage() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await fetch(`/api/accounting/vendor-payments/${selectedPayment.id}`, { method: 'DELETE' });
  ...
  const handleStatusUpdate = async (id: string, status: string) => {
    setUpdatingId(id);
    try {
      const res = await fetch(`/api/accounting/vendor-payments/${id}`, {
        method: 'PATCH',
  ...
  const filteredPayments = payments.filter(payment => {
    ...
    return (
      ...
      (payment.vendor?.name?.toLowerCase() || '').includes(q) ||
  ...
  const sortedPayments = [...currentPayments].sort((a, b) => {
    ...
    switch (sortField) {
      ...
      case 'vendor': aVal = a.vendor?.name || ''; bVal = b.vendor?.name || ''; break;
  ...
  const getStatusBadge = (status: string) => {
    if (status === 'Cleared' || status === 'cleared') {
  ...
  return (
    <div className="p-6 bg-gray-900 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-white">Manage Vendor Payments</h1>
          <p className="text-sm text-gray-400 mt-1">Manage payments made to vendors</p>
        </div>
        <Link href="/dashboard/accounting/vendor-payments/create">
              ...
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider cursor-pointer hover:text-white" onClick={() => handleSort('vendor')}>
                  <div className="flex items-center gap-1">
                    ...
                    Vendor <SortIcon field="vendor" sortField={sortField} sortDirection={sortDirection} />
                  </div>
                </th>
                  ...
                  <tr key={payment.id} className="hover:bg-gray-700/50 transition">
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-white">{payment.vendor?.name || '—'}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-400">{payment.bankAccount?.name || '—'}</span>
                      </div>
                    </td>
                    <td className="px-4 py-4 whitespace-nowrap">
          ...
          <div className="bg-gray-800 rounded-lg border border-gray-700 max-w-md w-full">
            <div className="p-6">
              ...
              <div className="bg-gray-900 rounded-lg p-3 mb-4">
                ...
                <p className="text-sm text-gray-400">Vendor: {selectedPayment.vendor?.name || '—'}</p>
                <p className="text-sm font-medium text-white">{formatCurrency(selectedPayment.amount)}</p>
              </div>
              <p className="text-sm text-red-400">This action cannot be undone.</p>
              <div className="mt-6 flex justify-end gap-3">
                <button onClick={() => { setShowDeleteModal(false); setSelectedPayment(null); }}
                  className="px-4 py-2 border border-gray-700 rounded-lg text-gray-400 hover:bg-gray-700 transition">Cancel</button>
                <button onClick={confirmDelete} disabled={deletingId === selectedPayment.id}
                  className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition flex items-center gap-2 disabled:opacity-50">
                  {deletingId === selectedPayment.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />}
                  Delete
                </button>
              </div>
            </div>
