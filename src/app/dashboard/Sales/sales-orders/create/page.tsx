...
type Quote = { id: string; number: string; customerName: string };
...
export default function CreateSalesOrderPage() {
  ...
  const selectedContact = (id: string) => contacts.find((c) => c.id === id);
  const selectedUser = users.find((u) => u.id === form.assignedUserId);
  ...
  async function handleSave() {
    ...
    try {
      const body = {
        ...
        quoteNumber: selectedQuote?.number || null,
        opportunityId: form.opportunityId || null,
        accountId: form.accountId || null,
        accountName: selectedAccount?.name || null,
        assignedUserId: form.assignedUserId || null,
        assignedUserName: selectedUser?.name || selectedUser?.email || null,
        ...
        billingContactName: form.billingContactId ? (selectedContact(form.billingContactId)?.firstName + " " + selectedContact(form.billingContactId)?.lastName).trim() : null,
        shippingContactId: form.shippingContactId || null,
        shippingContactName: form.shippingContactId ? (selectedContact(form.shippingContactId)?.firstName + " " + selectedContact(form.shippingContactId)?.lastName).trim() : null,
    ...
    <DashboardLayout title="Create Sales Order">
      <div className="p-6 max-w-6xl mx-auto">
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <div className="space-y-1.5">
                ...
                <Input placeholder="Enter order name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="h-10" />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <select value={form.quoteId} onChange={(e) => setForm({ ...form, quoteId: e.target.value })} className={selectCls}>
                  <option value="">Select Quote</option>
                  {quotes.map((q) => (<option key={q.id} value={q.id}>{q.number} - {q.customerName}</option>))}
                </select>
              </div>
              <div className="space-y-1.5">
              ...
              <div className="space-y-1.5">
                ...
                <select value={form.assignedUserId} onChange={(e) => setForm({ ...form, assignedUserId: e.target.value })} className={selectCls}>
        ...
        <Card className="mt-6">
          ...
          <CardContent>
            <textarea placeholder="Order description..." value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className={textareaCls} />
          </CardContent>
        </Card>
        ...
        <div className="flex gap-2 justify-end mt-6">
          <Button variant="outline" onClick={() => router.push("/dashboard/Sales/sales-orders")} disabled={saving}>
            Cancel
          </Button>
