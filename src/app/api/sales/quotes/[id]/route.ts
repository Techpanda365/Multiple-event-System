...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  for (const key of [
    "name", "customerName", "customerEmail", "customerId", "opportunityId", "opportunityName",
    "accountId", "accountName", "assignedUserId", "assignedUserName",
  ...
  if (body.quoteDate) data.quoteDate = new Date(body.quoteDate);
  if (body.expiryDate) data.expiryDate = body.expiryDate ? new Date(body.expiryDate) : null;

  const result = await prisma.salesQuote.updateMany({
    where: { id, workspaceId: ctx.workspace.id },
    data,
  });
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!ctx) return unauthorized();
  const { id } = await params;
  await prisma.salesQuote.deleteMany({ where: { id, workspaceId: ctx.workspace.id } });
  return Response.json({ success: true });
} catch (error) {
